...
export class CoursesService {
  ...
  async findOrderItemById(id: number, currentUserId: number) {
    return await this.orderItemsService
      .findOneQuery({
        courseId: id,
  ...
  // async findCartById(id: number, currentUserId: number) {
  //   return await this.cartsService
  //     .findOneQuery(
  //       {
  //         courseId: id,
  //       },
  ...
  async learnOrFailById(id: number, userId: number) {
    ...
    query = query.innerJoinAndMapOne(
      ...
      `${orderEntityName}a`,
      `${orderEntityName}a.status = :orderStatus AND ${orderEntityName}a.isActive = true AND ${orderEntityName}a.userId = :orderUserId`,
      {
        orderUserId: userId,
        orderStatus: EOrderStatus.Success,
      },
  ...
  findQuery(): SelectQueryBuilder<Course> {
    const query = this.courseRepository
      ...
      .leftJoin(`${orderItemEntityName}.${orderEntityName}`, orderEntityName)
      .andWhere(`${orderEntityName}.status = :orderStatusSuccess`, {
        orderStatusSuccess: EOrderStatus.Success,
      });
    ...
    if (purchase !== undefined && currentUserId) {
      query = query
        ...
        .innerJoin(
          ...
          `${orderEntityName}purchase.isActive = true AND ${orderEntityName}purchase.status = :orderStatus AND ${orderEntityName}purchase.userId = :boughtUserId`,
          {
  ...
  findOneQuery(
    ...
    const query = this.findQuery()
      ...
      .loadRelationCountAndMap(
        ...
        (qb) =>
          qb
            .andWhere(`${orderItemEntityName}.isActive = true`)
            .leftJoin(
              `${orderItemEntityName}.${orderEntityName}`,
              `${orderEntityName}`,
            )
            .andWhere(`${orderEntityName}.status = "${EOrderStatus.Success}"`),
      )
      .loadRelationCountAndMap(
    ...
    //   `${courseEntityName}.${orderItemEntityName}`,
    //   `${orderItemEntityName}`,
    //   (qb) => qb.andWhere(`${orderItemEntityName}.isActive = true`),
    //   // .innerJoin(
    //   //   `${orderItemEntityName}.${orderEntityName}`,
    //   //   orderEntityName,
    //   //   `${orderEntityName}.isActive AND ${orderEntityName}.paid = true AND ${orderEntityName}.userId = :userId`,
    //   //   { userId },
    //   // ),
    // );
